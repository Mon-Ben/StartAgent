...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, num_heads):
        ...
        self.d_model = d_model          # 模型的总维度（输入/输出特征维度）,输入序列中每个 token 的特征向量维度。在 Transformer 中通常为 512、768 等。
        self.num_heads = num_heads      # 并行的注意力头数量
        self.d_k = d_model // num_heads # 每个头的维度（必须能整除）

        # 定义 Q, K, V 和输出的线性变换层(batch_size, seq_len, d_model)
    ...
    def forward(self, Q, K, V, mask=None):
        # 1. 对 Q, K, V 进行线性变换;  注意在自注意力中 Q=K=V，但在交叉注意力中 Q 来自 decoder，K/V 来自 encoder。
